def plot_metrics(prefix, framework, save = False, show_all=False, history = None):
    ...
    if save:
        ...
        plt.clf()
        plt.plot(history.history['loss'][1000:]) # mean_squared_error
        plt.plot(history.history['val_loss'][1000:]) # val_mean_squared_error
...
def write_metrics(filename, framework, model_id, mse, msle, mape, fidelity):
    import os
    import csv
    ...
    if os.stat(filename).st_size == 0:
        writer.writerow(["Date", "Framework", "ID", "MSE", "MSLE", "MAPE", "Fidelity"])
    writer.writerow([date_string, framework, model_id, mse, msle, mape, fidelity])
    f.close()
...
def write_hls_report(filename, model_id, hls_results):
    import os
    import csv
